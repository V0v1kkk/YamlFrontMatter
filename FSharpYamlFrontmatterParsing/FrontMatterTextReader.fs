module FSharpYamlFrontmatterParsing.FrontMatterTextReader
...
type FrontMatterTextReader private (inner: TextReader) as this =
    ...
    let mutable currentLine: string | null = null
    ...
    let isDelimiter (line: string | null) =
        not (isNull line)
    ...
    override _.Read(buffer: char[], index: int, count: int) =
        ...
        this.Read(buffer.AsSpan(index, count))

    override _.Read(buffer: Span<char>) =
        if buffer.Length = 0 then
            0
        else
            let mutable written = 0

            while written < buffer.Length && ensureCurrentLine () do
                let source = currentLine.AsSpan(currentOffset)
                let toCopy = min source.Length (buffer.Length - written)

                source.Slice(0, toCopy).CopyTo(buffer.Slice(written))
